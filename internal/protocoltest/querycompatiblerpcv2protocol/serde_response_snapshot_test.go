...
func TestSerdeCheckResponseSnapshot_QueryCompatibleOperation(t *testing.T) {
	want := &QueryCompatibleOperationOutput{}
	status, header, body, err := serdeRespReadSnapshot("QueryCompatibleOperation")
	...
	got, err := svc.QueryCompatibleOperation(context.Background(), &QueryCompatibleOperationInput{})
	if err != nil {
		t.Fatal(err)
	}
	if err := smithytesting.CompareValues(want, got); err != nil {
		t.Errorf("response snapshot mismatch for %s: %v", "QueryCompatibleOperation", err)
	}
}
