...
func TestSerdeCheckResponseSnapshot_NonQueryCompatibleOperation(t *testing.T) {
	want := &NonQueryCompatibleOperationOutput{}
	status, header, body, err := serdeRespReadSnapshot("NonQueryCompatibleOperation")
	...
	got, err := svc.NonQueryCompatibleOperation(context.Background(), &NonQueryCompatibleOperationInput{})
	if err != nil {
		t.Fatal(err)
	}
	if err := smithytesting.CompareValues(want, got); err != nil {
		t.Errorf("response snapshot mismatch for %s: %v", "NonQueryCompatibleOperation", err)
	}
}
