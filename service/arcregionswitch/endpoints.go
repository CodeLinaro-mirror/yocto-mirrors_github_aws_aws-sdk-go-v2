...
import (
	...
	internalendpoints "github.com/aws/aws-sdk-go-v2/service/arcregionswitch/internal/endpoints"
...
func resolveBaseEndpoint(cfg aws.Config, o *Options) {
	...
	_, s := os.LookupEnv("AWS_ENDPOINT_URL_ARC_REGION_SWITCH")

	if g && !s {
		return
	}

	value, found, err := internalConfig.ResolveServiceBaseEndpoint(context.Background(), "ARC Region switch", cfg.ConfigSources)
	if found && err == nil {
...
type EndpointParameters struct {
	...
	// Whether the operation is a control plane operation. Control plane operations are
	// routed to a centralized endpoint in the partition leader.
	//
	// Parameter is
	// required.
	UseControlPlaneEndpoint *bool
}
...
func (r *resolver) ResolveEndpoint(
	...
	if exprVal := params.UseControlPlaneEndpoint; exprVal != nil {
		_UseControlPlaneEndpoint := *exprVal
		_ = _UseControlPlaneEndpoint
		if _UseControlPlaneEndpoint == true {
						...
						if exprVal := awsrulesfn.GetPartition(_Region); exprVal != nil {
							...
							if _PartitionResult.Name == "aws-cn" {
								uriString := func() string {
									...
									out.WriteString("https://arc-region-switch-control-plane.cn-north-1.")
				...
				if exprVal := params.Region; exprVal != nil {
					...
					if _UseFIPS == true {
						if exprVal := awsrulesfn.GetPartition(_Region); exprVal != nil {
							...
							_ = _PartitionResult
							if _PartitionResult.Name == "aws-cn" {
								return endpoint, fmt.Errorf("endpoint rule error, %s", "Invalid Configuration: FIPS is not supported in this partition")
							}
							uriString := func() string {
								var out strings.Builder
								out.WriteString("https://arc-region-switch-control-plane-fips.")
				...
				if !(_UseFIPS == true) {
					if !(params.Endpoint != nil) {
						if exprVal := awsrulesfn.GetPartition(_Region); exprVal != nil {
							...
							uriString := func() string {
								var out strings.Builder
								out.WriteString("https://arc-region-switch-control-plane.")
								out.WriteString(_PartitionResult.ImplicitGlobalRegion)
								out.WriteString(".")
								out.WriteString(_PartitionResult.DualStackDnsSuffix)
								return out.String()
	...
	if exprVal := params.Region; exprVal != nil {
		...
		if exprVal := awsrulesfn.GetPartition(_Region); exprVal != nil {
			...
			if _UseFIPS == true {
				uriString := func() string {
					var out strings.Builder
					out.WriteString("https://arc-region-switch-fips.")
			...
			uriString := func() string {
				var out strings.Builder
				out.WriteString("https://arc-region-switch.")
				out.WriteString(_Region)
				out.WriteString(".")
				out.WriteString(_PartitionResult.DualStackDnsSuffix)
				return out.String()
