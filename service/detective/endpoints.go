...
import (
	...
	internalendpoints "github.com/aws/aws-sdk-go-v2/service/detective/internal/endpoints"
...
func resolveBaseEndpoint(cfg aws.Config, o *Options) {
	...
	_, s := os.LookupEnv("AWS_ENDPOINT_URL_DETECTIVE")

	if g && !s {
		return
	}

	value, found, err := internalConfig.ResolveServiceBaseEndpoint(context.Background(), "Detective", cfg.ConfigSources)
	if found && err == nil {
				...
				if _UseFIPS == false {
					if _UseDualStack == true {
						uriString := func() string {
							...
							out.WriteString("https://detective.")
			...
			if _PartitionResult.Name == "aws" {
				if _UseFIPS == true {
					if _UseDualStack == true {
						uriString := func() string {
							var out strings.Builder
							out.WriteString("https://detective-fips.")
							out.WriteString(_Region)
							out.WriteString(".")
							out.WriteString(_PartitionResult.DualStackDnsSuffix)
							return out.String()
				...
				if _UseDualStack == true {
					if true == _PartitionResult.SupportsFIPS {
						if true == _PartitionResult.SupportsDualStack {
							uriString := func() string {
								var out strings.Builder
								out.WriteString("https://api.detective-fips.")
								out.WriteString(_Region)
								out.WriteString(".")
								out.WriteString(_PartitionResult.DualStackDnsSuffix)
								return out.String()
			...
			if _UseFIPS == false {
				if _UseDualStack == true {
					if true == _PartitionResult.SupportsDualStack {
						uriString := func() string {
							var out strings.Builder
							out.WriteString("https://api.detective.")
							out.WriteString(_Region)
							out.WriteString(".")
							out.WriteString(_PartitionResult.DualStackDnsSuffix)
							return out.String()
