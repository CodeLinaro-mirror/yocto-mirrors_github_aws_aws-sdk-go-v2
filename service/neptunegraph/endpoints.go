...
import (
	...
	internalendpoints "github.com/aws/aws-sdk-go-v2/service/neptunegraph/internal/endpoints"
...
func resolveBaseEndpoint(cfg aws.Config, o *Options) {
	...
	_, s := os.LookupEnv("AWS_ENDPOINT_URL_NEPTUNE_GRAPH")

	if g && !s {
		return
	}

	value, found, err := internalConfig.ResolveServiceBaseEndpoint(context.Background(), "Neptune Graph", cfg.ConfigSources)
	if found && err == nil {
...
func (p EndpointParameters) ValidateRequired() error {
	if p.ApiType == nil {
		return fmt.Errorf("parameter ApiType is required")
	}
				...
				if _UseDualStack == true {
					if true == _PartitionResult.SupportsFIPS {
						if true == _PartitionResult.SupportsDualStack {
							if _ApiType == "ControlPlane" {
								uriString := func() string {
									var out strings.Builder
									out.WriteString("https://neptune-graph-fips.")
							...
							if _ApiType == "DataPlane" {
								return endpoint, fmt.Errorf("endpoint rule error, %s", "Invalid Configuration: fips endpoint is not supported for this API")
							}
							return endpoint, fmt.Errorf("endpoint rule error, %s", "Invalid Configuration: Unknown ApiType")
						}
					}
			...
			if _UseDualStack == true {
				if true == _PartitionResult.SupportsDualStack {
					if _ApiType == "ControlPlane" {
						uriString := func() string {
							var out strings.Builder
							out.WriteString("https://neptune-graph.")
							out.WriteString(_Region)
							out.WriteString(".")
							out.WriteString(_PartitionResult.DualStackDnsSuffix)
							return out.String()
